...
#
#  기본값: SL 1.5×ATR, TP 2.4×ATR (RR 1.6). 손익분기 필요 승률 38.5%.
#  (2026-09-02 MFE 실측 156건으로 재확인 — FIX-TP2 참조)
...
# 🟥 [FIX-TP2] 2.0 → 2.4 되돌림 (2026-09-02). MFE 실측이 나온 뒤의 재판단.
#   FIX-TP1 에서 2.4→2.0 으로 줄인 건 MFE 가 없던 상태의 추정이었다. 이제 데이터가 있다.
#   Alpaca 156건 MFE 시뮬레이션 (보수 기준, 손절건 전환 없음 가정):
#       ATR×1.6 → −10.51%   ATR×2.0 → +0.01%   ATR×2.4 → +4.46%   ATR×2.8 → +9.34%
#   근거: 손절난 70건의 MFE/TP 중앙값이 18.5% 다. 지는 거래는 목표 쪽으로 아예 안 간다.
#         (목표의 10% 도 못 간 게 35.7%, 100% 넘긴 건 0건)
#         → TP 를 좁혀도 전환되는 게 거의 없고 승자만 깎인다.
#         반대로 TP 도달 53건의 MFE/TP 중앙값은 122.5% — 목표를 넘겨서 더 갔다.
#   ⚠️ 2.8 이 시뮬레이션 최고값이지만 n=156 그리드 최댓값이라 과적합 위험이 있다.
#      원래 값이던 2.4 로 되돌리는 데 그친다. 2.8 은 250건 시점에 사전등록 후 재검증.
STOCK_TP_ATR_MULT = float(os.getenv("STOCK_TP_ATR_MULT", "2.4"))
