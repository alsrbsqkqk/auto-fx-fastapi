...
async def webhook(request: Request):
    ...
    "BUY_ONLY_BREAKOUT_ENGULFING(Bar close)": -0.5,
    "SELL_ONLY_BREAKOUT_ENGULFING(Bar close)": -0.5,
    }
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "- 하지만 반드시 **현재가 기준으로 TP는 ATR기반으로 계산하되 과도한 목표 설정을 방지하기 위해, 계산식 TP distance는 min(ATR x2, 16 pip) 이 공식을 항상 따라라**, SL은 반드시 꼭 최근 ATR의 최소 1.1배 이상으로 설정하되 18pip 이내로 설정하게 해줘 어떻게 계산했는지도 보여줘. 예외는 없다 그렇지 않으면 시장 변동성 대비 손실 확률이 급격히 높아진다.\\n"
