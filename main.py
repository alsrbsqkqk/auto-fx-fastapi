...
async def webhook(request: Request):
    ...
    # 1️⃣ 기본 진입 조건: GPT가 BUY/SELL 판단 + 점수 3.0점 이상
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "- 너의 목표는 알림에서 울린 BUY 또는 SELL을 사전에 고정하지 않고, BUY 점수와 SELL 점수를 너가 각각 산출한 뒤 더 높은 점수를 최종 판단으로 선택하는 것이야. 그리고 승률이 60%이상이 넘는다고 생각하는 거래에만 BUY나 SELL결정을 내려줘. BUY 점수가 SELL 점수보다 높으면 'BUY', SELL 점수가 BUY 점수보다 높으면 'SELL', 두 점수가 같거나 모두 낮으면 'WAIT'으로 한다.\n"
                ...
                "(2) 거래는 기본적으로 1시간 내 청산을 목표로 하고 는 단타 트레이딩이다(최대 2시간), SL과 TP는 ATR의 가급적이면 최소 50% 이상 거리를 설정하되 너무 청산까지 오래 걸릴꺼 같으면 무시해. TP는 현재가에서 10 pip이내에 SL은 10 pip이내로 설정해줘\n"
                ...
                "(3) 지지선(support), 저항선(resistance)은 최근 1시간봉 기준 마지막 6봉의 고점/저점에서 이미 계산되어 JSON에 포함되어 있어. support와 resistance를 적절히 고려하되 TP/SL설정은 suppor resistance가 아무리 멀리 있어도 10 pip이내로 설정해. 명심할것은 지지선 저항선은 참고만하되 tp와 sl은 10 pip정도에서 머물러야해. 또한 너 결경이 BUY이면 TP는 현재가보다 높아야하고 SL은 현재가보다 낮게, SELL이면 TP는 현재가보다 낮아야 하고 sl은 현재가보다 높아야함. 이 규칙은 지키지 않으면 OANDA에서 거래 캔슬되니깐 반드시 지켜.  .\n"
