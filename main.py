...
async def webhook(request: Request):
    ...
    trend, signal, liquidity, pattern, pair, candles, atr, price, boll_up.iloc[-1], boll_low.iloc[-1]
    )
    # 🎯 뉴스 리스크 점수 추가 반영
    ...
    messages = [
        {"role": "system", 
         "content": (
            ...
            "모든 요소를 종합적으로 분석해서 진입 판단(BUY, SELL, WAIT)과 TP, SL 값을 제시해.\n"
            "signal_score가 4점 이상이면 보수적인 WAIT보다는 BUY 또는 SELL을 적극적으로 검토해.\n\n"
            ...
            "TP:SL 비율은 2:1 이상이면서 최소 10pip 이상 차이 나야 해. BUY일 땐 TP > 진입가, SL < 진입가 / SELL일 땐 TP < 진입가, SL > 진입가를 반드시 지켜.\n\n"
