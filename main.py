...
async def webhook(request: Request):
    ...
    "BUY_ONLY_BREAKOUT_ENGULFING_Bar close0219": -1.0,
    "SELL_ONLY_BREAKOUT_ENGULFING_Bar close0219": -1.0,
    }
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "- 하지만 반드시 **현재가 기준으로 TP는 ATR기반으로 계산하되 과도한 목표 설정을 방지하기 위해, 계산식 TP distance는 min(ATRx1.2, 0.11) 이 공식을 항상 따라라 tp는 항상 11PIP을 초과하지 않도록 한다.**, SL distance는 min(ATRx1.1, 0.13)이 공식을 항상 따라라 SL은 항상 13pip을 초과하지 않도록 한다. 이내로 설정하게 해줘 어떻게 계산했는지도 보여줘. 예외는 없다 그렇지 않으면 시장 변동성 대비 손실 확률이 급격히 높아진다.\\n"
