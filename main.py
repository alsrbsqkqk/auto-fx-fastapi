    ...
    messages = [
        {
            ...
            "content": (
                ...
                "(3) 지지선(support), 저항선(resistance)은 최근 1시간봉 기준 마지막 6봉의 고점/저점에서 이미 계산되어 JSON에 포함되어 있어. support와 resistance를 적절히 고려하되 TP/SL설정은 suppor resistance가 아무리 멀어도 10 pip이내로 설정해. 그리고 명심해야 할건 너 결경이 BUY이면 TP는 현재가보다 높아야하고 SL은 현재가보다 낮게, SELL이면 TP는 현재가보다 낮아야 하고 sl은 현재가보다 높아야함  .\n"
